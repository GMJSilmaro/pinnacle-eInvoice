...
router.post('/export-template', [auth.middleware], async (req, res) => {
    try {
        ...
        const excelData = line_items.map(item => {
            // Calculate tax amount based on the amount and tax rate
            const amount = parseFloat(item.amount || 0);
            const taxRate = parseFloat(tax_info.tax_rate || 0);
            ...
            return {
                ...
                CurrencyCode: currency_info.currency || 'MYR',
                TaxCurrencyCode: 'MYR',
                ExchangeRate: currency_info.exchange_rate || '1.0000',
                BillingPeriodStart: invoice_details.start_date,
                BillingPeriodEnd: invoice_details.end_date,
                BillingFrequency: 'Monthly',
                eInvoiceVersion: '1.0',
                eInvoiceType: '01',
                PaymentDueDate: invoice_details.end_date, // Set due date same as invoice date
                ...
                Classification: item.classification || '004',
                TaxType: item.tax_type || '01',
                TaxRate: tax_info.tax_rate || '8',
                TaxAmount: taxAmount,
                TaxExemptionReason: item.tax_type === '06' ? 'Out of scope of SST' : 
                                   item.tax_type === 'E' ? 'SST Exempted' : '',
                TaxExemptionCode: item.tax_exemption_code || '',
                DiscountAmount: item.discount_amount || '0.00',
                DiscountReason: item.discount_reason || '',
                TotalExclTax: amount.toFixed(2),
                ...
                // Buyer Information (General Public)
