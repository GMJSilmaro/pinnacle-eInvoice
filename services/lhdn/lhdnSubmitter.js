...
class LHDNSubmitter {
  ...
  async updateExcelWithResponse(fileName, type, company, date, uuid, longId, invoice_number) {
    try {
        ...
        // Extract InvoiceTypeCode from fileName
        const invoiceTypeCode = this.extractInvoiceTypeCode(fileName);

        // Retrieve longId from submission details
        const submissionDetails = await this.getSubmissionDetails(uuid, this.req.session.accessToken);
        const retrievedLongId = submissionDetails.success ? submissionDetails.longId : longId || "PENDING";
